...
There is no `check_*()` function namespace in this repository, and this section names none. Each clause above is carried by a ledger row in `ledger/rows.yaml`, asserted by that row's probe in `ledger/checks/test_custody_rows.py` or by the tests that row cites, and run by `make test` (which collects `ledger/checks` alongside every tier) and by CI. Each clause's own **Machine check:** line names the row that carries it.
