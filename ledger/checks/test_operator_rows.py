...
EXEMPTION_REGISTER: frozenset[str] = frozenset(
    {
        ...
        "webtheme.py:4275",  # {style}               -- axis ruler numeral offset
        "webtheme.py:4294",  # left:{_grad_x(f):.1f}px -- graduation tick offset
        "webtheme.py:4301",  # width:{px}px          -- age bar length
...
def test_row_osv1_031() -> None:
    ...
    assert len(red) == 3, (
        ...
        f"2026-09-05, work_item_pipeline-np3; OSV1-008 went green 2026-09-05, "
        f"work_item_pipeline-v3m.) MEASURED on this tree, not assumed: the remaining "
        f"red Core rows are OSV1-003, OSV1-010 and OSV1-012."
    )
