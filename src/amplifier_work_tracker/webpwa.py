...
Kept identical to muxplex, deliberately:
  ...
  - Icons served as real PNG bytes from a sibling `webassets/` directory,
    read once at import time -- the same `_FONTS_DIR` / `_font_b64`
    pattern `webtheme.py` uses for its two embedded fonts, applied here to
    files that must be independently fetchable HTTP resources (a manifest
    icon URL, an `apple-touch-icon` link) rather than inlined into CSS.
    Generated by `scripts/gen_pwa_icons.py` -- see that script's own
    docstring for the mark and why it looks the way it does.
...
`webapp.py` wires the five routes this module's content is served through
(`/manifest.json`, `/sw.js`, `/pwa-192.png`, `/pwa-512.png`,
`/apple-touch-icon.png`) and exempts them from PAM auth -- a browser must
be able to fetch the manifest, the service worker, and the icons before
(and independent of) any login, or "Add to Home Screen" / install never
works. None of the bytes this module serves are sensitive.
"""
...
def icon_bytes(filename: str) -> bytes:
    """Read one of the three PWA icon PNGs from `webassets/`.

    `filename` must be one of "pwa-192.png", "pwa-512.png",
    "apple-touch-icon.png" -- the routes in `webapp.py` pass a fixed
    literal, never client input, so no path-traversal handling is needed
    here.
    """
