...
(`resolve` on an already-resolved item is a readback-checked no-op ONLY when
the text you send is byte-for-byte what is already stored -- which is
exactly the retry case; sending DIFFERENT text against a closed item now
fails non-zero having written nothing, and names `reopen` as the remedy) --
but re-reading first is always the safe move. See `context/awareness.md` for
