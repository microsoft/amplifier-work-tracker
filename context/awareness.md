...
You're one of several agents pulling from a shared work queue. Four things
here fail **silently** if you get them wrong — no error, no undo:

1. **Claim only via `work_claim` / `work_status` reports; never list-then-pick.**
   The obvious approach — read the ready queue, choose an item, mark it
   yours — double-claimed in **2–5 of every 6 measured trials**, and every
   losing agent still got exit 0. `work_claim` is the single atomic
   claim-and-custody operation. There is no other way to take an item.
...
- Turning a raw user report into a properly-specified engineering issue →
  delegate to `work-tracker:feedback-triage`.
- Need the full claim/custody procedure, including what to do after a reap →
  `load_skill(skill_name="claiming-work-safely")`.
- Need to read `doctor` output, understand the seam, or schedule
  `reap`/`notify` →
